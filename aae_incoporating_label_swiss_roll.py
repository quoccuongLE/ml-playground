...
def main(
    ...
    if os.environ.get("LAUNCH_MODE") == "debug":
        weight_dir = Path(f"tmp/weights/aae/swiss_roll/e{num_epochs}/debug")
    else:
        weight_dir = Path(f"tmp/weights/aae/swiss_roll/e{num_epochs}/s{seed}")
    ...
    batch_errD, batch_errR, batch_errG = float("inf"), float("inf"), float("inf")
